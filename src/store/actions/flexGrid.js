...
export const calcUnits = () => async (dispatch, getState, { flexGridInstance }) => {
  ...
  if (variantId) {
    ...
    if (!invalid.length) {
      ...
      const mainDirectionIndex = getMainDirectionIndex(mainDirectionSigns)
      window.explorerBridge.variantResult(variantId, { units, directionNames, mainDirectionIndex })
